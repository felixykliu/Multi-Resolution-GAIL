...
class PROG_RNN(nn.Module):
    ...
    def forward(self, data, step_size, keep_dim=False):
        # data: seq_length * batch * 10
        loss = Variable(torch.zeros(data.size(1), data.size(2)))
        if self.params['cuda']:
            loss = loss.cuda()
        count = 0
        ...
        if keep_dim is False:
            return torch.sum(loss) / count / data.shape[1]
        else:
    ...
    def get_log_prob_traj(self, data):
        # data: seq_length * batch * 10, CAN'T be backpropogated through
        nll = Variable(torch.zeros(data.size(1)))
        if self.params['cuda']:
            nll = nll.cuda()
        
        step_size = 50 // (data.shape[0] - 1)
        
        while (data.shape[0] > 3):
            nll += self.forward(data, step_size, keep_dim=True)
            data = data[::2]
            step_size *= 2
