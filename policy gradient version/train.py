...
parser.add_argument('--continue_current_training', action='store_true', default=False, help='continue current training')
...
for e in range(pretrain_epochs):
    ...
    _, _, _, _, _, _, _, mod_stats, exp_stats = \
            collect_samples(policy_net, discrim_net, test_data, use_gpu, e, args.subsample, name='pretrain', draw=True)
...
if args.continue_current_training:
    print("loading current policy and discriminator ...")
    policy_state_dict = torch.load(save_path+'model/policy_step'+str(args.subsample)+'_training.pth')
    policy_net.load_state_dict(policy_state_dict)
    discrim_state_dict = torch.load(save_path+'model/discrim_step'+str(args.subsample)+'_training.pth')
    discrim_net.load_state_dict(discrim_state_dict)    
...
for i in range(args.pretrain_disc_iter):
    exp_states, exp_actions, exp_seq, model_states, model_actions, model_seq, rewards, mod_stats, exp_stats = \
        collect_samples(policy_net, discrim_net, train_data, use_gpu, i, args.subsample, name="pretraining", draw=False)
...
for i_iter in range(args.max_iter_num):
    '''
    if i_iter == args.max_iter_num // 2:
        optimizer_policy = torch.optim.Adam(
            filter(lambda p: p.requires_grad, policy_net.parameters()),
            lr=args.policy_learning_rate//10)
    '''

    ts0 = time.time()
    print("Collecting Data")
    exp_states, exp_actions, exp_seq, model_states, model_actions, model_seq, rewards, mod_stats, exp_stats = \
        collect_samples(policy_net, discrim_net, train_data, use_gpu, i_iter, args.subsample, draw=False)
    
    # draw and stats
    if i_iter % args.draw_interval == 0:
        test_fixed_data(policy_net, fixed_test_data, 'fixed_test', i_iter, args.subsample, 1)
        _, _, _, _, _, _, _, mod_stats, exp_stats = \
            collect_samples(policy_net, discrim_net, test_data, use_gpu, i_iter, args.subsample, draw=True)
    ...
    if i_iter > 3 and mod_p[-1] < 0.8:
        update_policy(policy_net, optimizer_policy, discrim_net, discrim_criterion, model_seq, rewards, i_iter, use_gpu)
    t1 = time.time()
